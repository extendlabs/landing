...
export default function DecorativeCubes({
  ...
  return (
    <div className={`relative ${className}`}>
      {/* Glow effect */}
      <div
        className={`absolute inset-0 bg-gradient-to-r ${selectedColors.glow} blur-3xl opacity-20 rounded-full lg:scale-150 -z-10`}
        aria-hidden="true"
      />

      {/* Main content container */}
      <div className="relative size-60 md:w-80 md:h-80 lg:w-96 lg:h-96 flex items-center justify-center">
        {/* Center content (logo or image) */}
        <div className="relative z-10 animate-float">{children}</div>

        {/* Decorative Cube 1 - Top Right (Closer) */}
        <div
          className={`absolute top-6 right-6 md:top-22 md:right-22 size-20 md:w-14 md:h-14 lg:w-22 lg:h-22 border-2 ${selectedColors.cube1} rounded-lg animate-float-delayed-rotated opacity-60 backdrop-blur-sm`}
        ...
        <div
          className={`absolute bottom-6 left-6 md:bottom-22 md:left-22 size-20  md:w-14 md:h-14 lg:w-22 lg:h-22 border-2 ${selectedColors.cube2} rounded-lg animate-float-delayed opacity-60 backdrop-blur-sm`}
        ...
        <div
          className={`absolute top-6 left-6 md:top-22 md:left-22 size-20  md:w-14 md:h-14 lg:w-22 lg:h-22 border-2 ${selectedColors.cube3} rounded-lg animate-float-delayed opacity-60 backdrop-blur-sm`}
          style={{ animationDelay: "1s" }}
          aria-hidden="true"
        ></div>
        ...
        <div
          className={`absolute bottom-6 right-6 md:bottom-22 md:right-22 size-20  md:w-14 md:h-14 lg:w-22 lg:h-22 border-2 ${selectedColors.cube4} rounded-lg animate-float-delayed opacity-60 backdrop-blur-sm`}
          style={{ animationDelay: "1s" }}
          aria-hidden="true"
        ></div>
      </div>
